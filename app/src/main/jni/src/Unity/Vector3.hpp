...
char Vector3::ToChar(Vector3 a) {
    char buffer[64];
    // Safely format the vector into a string. 
    // This replaces the dangerous (const char*) cast.
    snprintf(buffer, sizeof(buffer), "%.1f, %.1f, %.1f", a.X, a.Y, a.Z);
    
    // Returning index 24 (the last element) or buffer[0].
    // Note: returning buffer[25] on a 25 size array is out of bounds.
    return buffer[0]; 
}
